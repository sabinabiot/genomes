...
genbank_records = []
contig_num = 0
for genbank_record in SeqIO.parse("%s/%s.gbk" % (args.prokka_dir, args.prokka_prefix), "genbank"):
    contig_num += 1
    genbank_record.id = "%s_ctg%s" % (args.output_prefix, str(contig_num).zfill(4))
    genbank_record.name = "%s_ctg%s" % (args.output_prefix, str(contig_num).zfill(4))
